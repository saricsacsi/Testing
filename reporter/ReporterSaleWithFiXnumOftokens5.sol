...
// owners: 009ecd6b08d0798c10f491c78d6d0d6e9c600919 AND 004c2db4e3721c6bf5edb49767ef003ecb68e00a
  ...
  // how many token units a buyer gets per wei IF THE FUCKING DECIMALS IS 18, but not now!
  uint256 public wtr1 = 250*10**6;  
  // wei/token rate only now because the decimals is just 2, and the its calculated from totalsupply and price
 ...
 // fallback function can be used to buy tokens
  ...
  function buyTokens(address beneficiary, uint256 weiAmount) internal {
   ...
   //uint256 actualRate = getRateAt(getCurrentTimestamp()); 
    ...
    // calculated the amount of tokens what i want to send
    // only now, because the fucking decimals is 2

    uint256 value = weiAmount.div(wtr1); 
