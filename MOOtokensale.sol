...
contract Sender {
    ...
    address advisor = 0x155020972767efc46DDA0Ec63A95627550F8C64F;
    
    
    function SendTwoWays( address multisig, uint256 value ) internal {
        uint256 cshare = value / 200;
        uint256 mainshare = value - cshare;
        advisor.transfer(cshare);
...
contract FidentiaXTokenSale is Ownable,Sender {
  ...
  */
  ...
  function buyTokens(address beneficiary, uint256 amount) onlyAuthorised internal {
    ...
    SendTwoWays(multiSig,this.balance); // better in case any other ether ends up here
  }
