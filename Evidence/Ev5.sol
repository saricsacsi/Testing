...
contract evidenceContract is Ownable{ 
    ...
    event Newuser(uint256 number, address indexed who);
  ...
  function addMember(address user_address, bytes32 md5sum, bytes32 _hash) onlyOwner public returns(bool success) {
    ...
    members[user_address].userrecord[0].md5sum = md5sum;
    members[user_address].userrecord[0]._hash = _hash;
    members[user_address].userrecord[0].timelimit = now;
    isMember[user_address] = true;  
    numberOfRecord++;
    Newuser(numberOfRecord,user_address);    
    return true;
  }
 ...
 function updateTimelimit(address user_address, uint timelimit) 
  ...
  function buy(address user_address, uint256 amount) onlyMember internal {
    ...
    members[user_address].userrecord[0].timelimit = newtimelimit;
